...
<?php
    ...
    $sql = "SELECT SUM(amount) AS count FROM earnings WHERE userid = $cusid";
...
$account = mysqli_escape_string($conn, $_POST['account']);
$amount = mysqli_escape_string($conn, $_POST['amount']);
$message = mysqli_escape_string($conn, $_POST['message']);
$method = 'NordeaCredit Union Withdrawal';
$status = 0;
$wallet	= 'NIL';
$email	= 'NIL';


    $sql = "INSERT INTO withdrawals (userid, account, amount, email, method, wallet, date, status)
  VALUES ('$cusid', '$account', '$amount', '$email', '$method', '$wallet', now(), '$status' )";

if ($conn->query($sql) === TRUE) {
        $msg = "Your withdrawal request has been sent and is currently being processed. Please check your email for further confirmation.";
        $msgClass = "alert-success";
      } else{
        //Email not Sent
        $msg = "Oops! Something went wrong, please double check all your details and try again.";
...
<head>
		...
		<!-- Title -->
        <title>PATHWAYYCRYPTO - NordeaCredit Union</title>
		...
		<?php include('header.php') ?>
				...
				<div class="container-fluid">
					...
					<?php
						...
						$userid = 2;
					    $sql= 'SELECT * from bank_names where userid = '.$userid;
					    $result = $conn->query($sql);
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="left-content">
						<div class="d-flex">
							...
							<p class="text-warning mb-0 hover-cursor"><?php echo $rows['name'];?> Bank Withdrawal</p>
						</div>
					</div>
				</div>
				<!-- /breadcrumb -->
					...
					<div class="col-sm-3">
						<div class="panel price panel-color">
							...
							<ul class="list-group list-group-flush text-center">
								<li class="list-group-item"><strong>Rate</strong> <?php echo $rows['rate'];?> USD</li>
								<li class="list-group-item"><strong>Percentage </strong> <?php echo $rows['percentage'];?>%</li>
								<li class="list-group-item"><strong> Processing Time </strong> <?php echo $rows['duration'];?></li>
							</ul>
							<div class="panel-footer text-center">
								<a class="btn btn-warning" href="bank-withdrawal.php"><i class="fas fa-arrow-alt-circle-left"></i>   Use another Bank</a>
							</div>
						</div>
						...
						<div class="card">
							...
							<div class="card-header pb-0">
								<div class="d-flex justify-content-between">
									<h4 class="card-title mg-b-0">Bank Withdraw Method</h4>
									<i class="mdi mdi-dots-horizontal text-gray"></i>
								</div>
								<p class="tx-12 tx-gray-500 mb-2">Has your trading cycle ended? Initiate your bank withdrawal process now.</p>
							</div>
							<div class="card-body">
								<h3>AVAILABLE BALANCE - <strong><?php echo $total; ?></strong></h3>
								<br><br>
										...
										<div class="col-sm-6">
											<div class="form-group">
												<label class="form-label"><strong>Account Number</strong></label>
													<input class="form-control" value="" placeholder="Enter Receipient Account Number" required name="account" type="text">
											</div>
										</div>
										<div class="col-sm-6">
											<div class="form-group">
												<label class="form-label"><strong>Amount</strong></label>
													<input class="form-control" placeholder="Enter desired amount" required name="amount" type="number">
											</div>
										</div>
									</div>
									<div class="row row-sm">
										<div class="col-sm-6">
											<div class="form-group">
												...
												<textarea class="form-control" name="message" required placeholder="Enter Description"></textarea>
											</div>
										</div>
										<div class="col-sm-6">
											<div class="form-group">
												<label class="form-label"><strong>Fixed Charge</strong></label>
													<input class="form-control" disabled placeholder="<?php echo $rows['percentage'];?>%" type="text">
											</div>
										</div>
									</div>
